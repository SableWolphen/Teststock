...
const signal=await read(SIGNAL,{}),watch=await read(WATCH,{schemaVersion:1,positions:[]}),previous=await read(OUT,null),cryptoAdmission=await read('docs/data/crypto-profitability-admission.json',{}),cryptoTournament=await read('docs/data/crypto-tournament.json',{});
...
const cryptoCandidates=buildCryptoMonitorCandidates({cryptoOrders,tournament:cryptoTournament});
const activePositions=(watch.positions||[]).filter(x=>x&&x.status==='ACTIVE');
const activeCryptoPositions=activePositions.filter(x=>x.assetClass==='CRYPTO').length;
const stockSymbols=unique([...stockCandidates.map(x=>x.ticker),...activePositions.filter(x=>x.assetClass==='STOCK').map(x=>x.ticker)]);
const cryptoSymbols=unique([...cryptoCandidates.map(x=>x.ticker),...activePositions.filter(x=>x.assetClass==='CRYPTO').map(x=>x.ticker)]);
const prices={},errors=[];
...
const tournamentAgeMinutes=ageMin(cryptoTournament.generatedAt);
const cryptoGenerationDeltaMs=Math.abs(Date.parse(signal.generatedAt||0)-Date.parse(cryptoTournament.generatedAt||0));
const cryptoSourceFresh=signalFresh&&tournamentAgeMinutes<=25&&Number.isFinite(cryptoGenerationDeltaMs)&&cryptoGenerationDeltaMs<=5*60_000;
...
for(const [i,x] of stockCandidates.entries()){
  ...
  else if(!actionAllowed&&dayTradeSeedLaneEligible&&!admissionOk){
