...
for(const [i,x] of stockCandidates.entries()){
  ...
  // seedLane.maxOrderUsd. It still requires per-order human approval and every other live guard;
  ...
  if(!actionAllowed&&!seedLaneEligible){status='BLOCKED_UPSTREAM';reason=`Candidate action ${x.action||'UNKNOWN'} is not eligible for monitoring.`;}
    ...
    else if(p>=min&&p<=max){status='SEED_LANE_BUY_TRIGGER';reason=`Seed-lane eligible SHADOW_ONLY candidate is inside its buy zone. Capped at $${Number(x.seedLane?.maxOrderUsd||5)}, part of up to ${Number(x.seedLane?.maxConcurrentPositions||5)} concurrent seed positions, still requires the user's per-order approval.`;}
  }
