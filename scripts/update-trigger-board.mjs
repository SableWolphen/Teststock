...
for(const [i,x] of stockCandidates.entries()){
  ...
  const admission=x.profitabilityAdmission?.state||'UNKNOWN',decisionEligible=x.decisionIntelligence?.eligibleAfterOverlay===true,actionAllowed=['AUTO_BUY_ELIGIBLE','WAIT_FOR_TRIGGER'].includes(String(x.action||''));
  // Stock seed lane (2026-08-26): apply-profitability-admission.mjs flags at most one SHADOW_ONLY
  // candidate as seedLane.eligible. It is otherwise blocked upstream (action stays
  // PROFITABILITY_ADMISSION_BLOCK, never loosened) -- this only lets that ONE flagged candidate
  // continue through the normal freshness/price checks below so it can surface a distinct
  // SEED_LANE_BUY_TRIGGER (never BUY_TRIGGER) capped at seedLane.maxOrderUsd. It still requires
  // per-order human approval and every other live guard; concurrency/pause are enforced live by
  // the execution-check against real-trade-journal.json, not here.
  ...
  if(!actionAllowed&&!seedLaneEligible){status='BLOCKED_UPSTREAM';reason=`Candidate action ${x.action||'UNKNOWN'} is not eligible for monitoring.`;}
    ...
    if(!signalFresh){status='REFRESHING_SIGNAL';reason='Current research generation is being refreshed. This candidate is display-only until a fresh generation arrives; no buy trigger may fire from aged research.';}
    ...
    else if(p>=min&&p<=max){status='SEED_LANE_BUY_TRIGGER';reason=`Seed-lane eligible SHADOW_ONLY candidate is inside its buy zone. Capped at $${Number(x.seedLane?.maxOrderUsd||25)}, one concurrent seed position maximum, still requires the user's per-order approval.`;}
  }
  ...
  items.push(stableItem({id:`ENTRY:STOCK:${x.ticker}`,kind:'ENTRY',assetClass:'STOCK',ticker:x.ticker,entryTier:tier,entryTierLabel:x.entryTierLabel||(tier==='B'?'BEST_ACCEPTABLE':'ELITE'),entryTierSizeMultiplier:Number(x.entryTierSizeMultiplier??(tier==='B'?.5:1)),queueRank:Number(x.queueRank??x.rank??i+1),opportunityScore:Number(x.decisionScore??x.portfolioOpportunityScore??x.opportunityScore??x.growthQuality??0),growthQuality:Number(x.growthQuality||0),rewardRisk:Number(x.rewardRisk||0),profitabilityAdmission:admission,decisionIntelligenceEligible:decisionEligible,seedLaneEligible,seedLane:x.seedLane||null,minimumEntry:min,maximumEntry:max,stop:Number(x.stop),target1:Number(x.target1),target2:Number(x.target2),signalGeneratedAt:signal.generatedAt||null},status,p,reason));
}
for(const [i,x] of cryptoOrders.entries()){
  ...
  if(!signalFresh){status='REFRESHING_SIGNAL';reason='Current crypto generation is being refreshed.';}else if(!(p>0)){status='PRICE_UNAVAILABLE';reason='Latest Alpaca crypto price unavailable.';}else if(p>max){status='DO_NOT_CHASE';reason='Price is above maximumEntry.';}else if(p>=min&&p<=max){status='DIRECT_API_ENTRY_ZONE';reason='Crypto is inside its research entry zone, but its separate direct API lane—not Claude dispatch—owns execution.';}
  items.push(stableItem({id:`ENTRY:CRYPTO:${x.ticker}`,kind:'ENTRY',assetClass:'CRYPTO',ticker:x.ticker,queueRank:Number(x.rank||i+1),opportunityScore:Number(x.opportunityScore??x.growthQuality??0),growthQuality:Number(x.growthQuality||0),rewardRisk:Number(x.rewardRisk||0),minimumEntry:min,maximumEntry:max,stop:Number(x.stop),target1:Number(x.target1),target2:Number(x.target2),signalGeneratedAt:signal.generatedAt||null},status,p,reason));
}
...
const actionable=new Set(['BUY_TRIGGER','SEED_LANE_BUY_TRIGGER','TRIGGER_1_STOP','TRIGGER_2_TARGET1','TRIGGER_3_TARGET2']);
const events=items.filter(x=>actionable.has(x.status)).map(x=>({id:x.id,assetClass:x.assetClass,ticker:x.ticker,trigger:x.status,entryTier:x.entryTier??null,entryTierLabel:x.entryTierLabel??null,entryTierSizeMultiplier:x.entryTierSizeMultiplier??null,queueRank:x.queueRank??null,opportunityScore:x.opportunityScore??null,growthQuality:x.growthQuality??null,rewardRisk:x.rewardRisk??null,profitabilityAdmission:x.profitabilityAdmission??null,decisionIntelligenceEligible:x.decisionIntelligenceEligible??null,seedLane:x.seedLane??null,observedPrice:x.observedPrice,stateChangedAt:x.stateChangedAt,reason:x.reason}));
...
const meaningfulChanged=!previous||oldHealth!==monitorHealth||JSON.stringify((previous.items||[]).map(x=>[x.id,x.status,x.observedPrice,x.entryTier]))!==JSON.stringify(items.map(x=>[x.id,x.status,x.observedPrice,x.entryTier]));
...
const out={schemaVersion:4,source:'TESTSTOCK_NON_LLM_TRIGGER_MONITOR',publishedAt,monitorHealth,monitorCadenceMinutes:5,priceSource:'ALPACA_MARKET_DATA',claudeMarketPollingRequired:false,siteAvailability:'24_7_PUBLIC_DASHBOARD',researchState:signalFresh?'ACTIVE':'REFRESHING',researchAgeMinutes:Number.isFinite(signalAgeMinutes)?Number(signalAgeMinutes.toFixed(1)):null,displayPolicy:'Never present aged research as an active trading signal. During refresh, keep the latest candidate visible as display-only and block new buy triggers until a fresh generation arrives.',stockSessionRule:'Stock entry eligibility can close with the market; the site and monitoring health remain independently visible.',executionWakeBridge:{status:'NOT_AVAILABLE_IN_THIS_REPOSITORY',note:'This monitor detects triggers without an LLM and publishes them. It cannot itself wake Claude or place Robinhood orders. A Claude run or another authorized execution bridge is still required to act.'},watchlist:{activePositions:activePositions.length,armingRule:'After a confirmed Robinhood buy, the execution agent must add the saved stop/target plan to docs/data/execution-watchlist.json. If it cannot arm this watchlist, do not treat fractional synthetic protection as active.'},executionNeeded:events.length>0,buyCompetition:{eligibleNow:buyCompetition.length,best:buyCompetition[0]||null,fallbacks:buyCompetition.slice(1),tierPriority:['A','B'],candidateSource:'stockPlan.stockCandidateQueue + stockPlan.stockOrders',rule:'A/ELITE live stock triggers go first. If no A survives the live broker guards, B/BEST_ACCEPTABLE may be executed at its reduced encoded size. Failed candidates fall through to the next eligible candidate in the same run. Never force a trade when all hard guards fail.'},events,items,errors:errors.length?errors:[],triggerDefinitions:{BUY_TRIGGER:'Qualified fresh entry zone reached; live broker checks still required.',SEED_LANE_BUY_TRIGGER:'The single seed-lane-eligible SHADOW_ONLY candidate reached its entry zone. Capped at seedLane.maxOrderUsd, one concurrent seed position maximum, still requires the user\'s per-order approval like every other stock buy -- never auto-submitted.',TRIGGER_1_STOP:'Stop/invalidation reached — verify position and sell protected quantity.',TRIGGER_2_TARGET1:'Target 1 reached — execute validated first scale-out.',TRIGGER_3_TARGET2:'Target 2 reached — execute validated exit/runner decision.'},creditPolicy:'Alpaca/GitHub monitor prices. Claude should read the small dispatch packet first and avoid market/broker work when no action is needed.'};
