...
function adaptiveSymbolStats(history,symbol){
  ...
  return {samples:rows.length,wins,losses,winRate,sizeMultiplier,qualityAdjustment,label,source:'MODEL_RESEARCH_ONLY'};
}
...
function researchOutcomeDiagnostics(history){
...
for(const budget of budgets){
  ...
  const eligible=ranked.filter(r=>r.direction==='BULLISH'&&r.growthQuality>=85&&r.rewardRisk>=2.0&&Number(r.expectancy?.costAdjustedConservativeExpectedR??r.expectancy?.conservativeExpectedR??0)>=.35&&Number(r.validation?.samples||0)>=12&&Number(r.validation?.winRate||0)>=50&&r.adaptivePerformance?.label!=='COLD'&&data.learning?.sector?.confirm!==false&&data.learning?.intraday?.confirm!==false).slice(0,8);
  ...
  for(const r of eligible){
    ...
    const amount=round(Math.max(0,Math.min(remainingBudget,desired*adaptiveSize,byRisk)));
