...
const MIN_SHADOW=12, MIN_REAL_PROBATION=8, MIN_REAL_FULL=15;
...
function admission(row){
  ...
  return {state,sizeMultiplier,setupType:setup,runtimeRegime:regime,shadow:s,real:{samples:Number(r.samples||0),winRatePct:r.winRatePct??null,averageRealizedR:r.averageRealizedR??null},thresholds:{minimumShadowSamples:MIN_SHADOW,minimumShadowWinRatePct:SHADOW_MIN_WIN,minimumShadowAverageR:SHADOW_MIN_AVG_R,minimumRealSamplesForSuspensionCheck:MIN_REAL_PROBATION,minimumRealSamplesForFullAdmission:MIN_REAL_FULL,minimumRealAverageRForFullAdmission:REAL_MIN_AVG_R},reason};
}
...
tournament.profitabilityAdmissionPolicy={enabled:true,mode:'SHADOW_FIRST_THEN_REDUCED_LIVE_PROBATION',rule:'Unproven patterns stay shadow-only. Positive shadow evidence unlocks reduced-size live probation. Full normal eligible size requires sufficient positive Robinhood-confirmed real fills. Negative probation evidence suspends the pattern back to shadow-only. This gate may reduce or block risk but never increase the maximum risk ceiling or loosen any hard safety rule.'};
