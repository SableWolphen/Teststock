...
const candidates=Object.keys(policies).filter(id=>id!==baseline).map(id=>({id,dev:summary[id].DEVELOPMENT,hold:summary[id].HOLDOUT_2024_2025,confirm:summary[id].CONFIRMATION_2026_PLUS})).filter(x=>x.dev.samples>=40&&x.hold.samples>=20).sort((a,b)=>(b.dev.avgR??-99)-(a.dev.avgR??-99));
let recommended=baseline,reason='Baseline retained because no runner policy cleared untouched holdout requirements.';
for(const c of candidates){const baseH=summary[baseline].HOLDOUT_2024_2025,baseC=summary[baseline].CONFIRMATION_2026_PLUS;const holdOk=(c.hold.avgR??-99)>=(baseH.avgR??-99)&&c.hold.maxDrawdownR<=baseH.maxDrawdownR*1.15;const confirmSparse=c.confirm.samples<12;const confirmOk=confirmSparse||((c.confirm.avgR??-99)>=(baseC.avgR??-99)&&c.confirm.maxDrawdownR<=Math.max(1,baseC.maxDrawdownR*1.2));if(holdOk&&confirmOk){recommended=c.id;reason=`${c.id} improved or matched untouched holdout average R without materially worsening drawdown${confirmSparse?'; 2026+ confirmation is still sparse so live risk may not increase':''}.`;break;}}
const out={schemaVersion:1,generatedAt:new Date().toISOString(),symbolsTested:symbols.length,method:'Walk-forward daily-bar exit-policy tournament. Policy selection is based on pre-2024 development only and must survive untouched 2024-2025 holdout; 2026+ is a second confirmation set. Results are research estimates, not guaranteed live fills.',baselinePolicy:baseline,recommendedPolicy:recommended,recommendationReason:reason,automaticRiskIncreaseAllowed:false,policies,summary,instructions:'Use this report only to choose among exit shapes. Never increase entry size or loosen stops because an exit policy backtested well. Missing/stale evidence falls back to the no-runner baseline.'};
await fs.writeFile(OUT,JSON.stringify(out,null,2));console.log(`Exit policy validation: ${rows.length} policy observations; recommended ${recommended}.`);
