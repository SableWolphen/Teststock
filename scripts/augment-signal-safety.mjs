...
const probabilityFile=path.resolve('docs/data/probability-first-policy.json');
...
const gateValidation=await read(validationFile),shadow=await read(shadowFile),probabilityPolicy=await read(probabilityFile);
const now=Date.now();
...
signal.probabilityFirstPolicy=probabilityPolicy||{status:'UNAVAILABLE',instructions:'Probability-first overlay is unavailable. Do not compensate by increasing risk; use the smaller existing Teststock limits.'};
signal.systemHealth={
  ...
  probabilityFirstOverlay:{status:probabilityPolicy?'AVAILABLE':'UNAVAILABLE',ageMinutes:ageMinutes(probabilityPolicy?.generatedAt),required:true},
  ...
  criticalDependencies:['fresh Teststock signal','probability-first overlay','live Robinhood equity/cash','known order/position state','verified protective-exit capability for a new stock/option','journal reconciliation or reliable Robinhood-history reconstruction'],
...
signal.performanceScalingGuard={
  ...
  scaleUnlock:{minimumResolvedRealTrades:20,minimumAverageRealizedR:0.20,minimumWinRatePct:45,maximumReliableObservedDrawdownPct:6,requirements:'SCALE risk may be used only when all available reliable real-fill metrics meet these minimums and no real-fill, slippage, protection, circuit-breaker, health, or persistence guard is blocking risk.'},
  growthUnlock:{minimumResolvedRealTrades:30,minimumAverageRealizedR:0.30,minimumWinRatePct:48,maximumReliableObservedDrawdownPct:6,requirements:'GROWTH risk may be used only when all available reliable real-fill metrics meet these stronger minimums and no real-fill, slippage, protection, circuit-breaker, health, or persistence guard is blocking risk.'},
...
signal.hardRules=[...(signal.hardRules||[]),
  ...
  'CRITICAL DEPENDENCY FAIL-CLOSED: if the Teststock signal, probability-first overlay, Robinhood account/order/position state, protective-exit capability, or journal/history reconciliation is stale, missing, contradictory, or unknown, open no new position. Manage existing exposure only.',
  ...
  'PROBABILITY-FIRST STOCKS: stocks are the primary growth engine. The probability-first overlay tightens stock evidence to at least 20 historical samples, 55% historical win rate, 0.60 conservative expected R, 2.5:1 reward/risk, and controlled volatility. It may reduce allocation and planned stop risk but may never loosen a failed base gate.',
  'OPTIONS + CRYPTO EVIDENCE UNLOCK: before a new option or crypto position, Claude must confirm at least 10 resolved real Teststock trades, average realized R >= 0.25, and win rate >= 50%, in addition to every research, tier, broker, whole-contract, event, spread, and protection rule. If the real-fill evidence is weaker or unavailable, use stocks/cash instead.'
];
...
console.log(`Augmented signal schema v17; performance-gated scaling and probability-first overlay enabled; robustness=${signal.entryGateRobustness.status}`);
