...
const assets=await getRetry('https://paper-api.alpaca.markets/v2/assets?status=active&asset_class=us_equity');
const universe=(assets||[]).filter(a=>a.status==='active'&&a.tradable===true&&a.symbol&&looksLikeOperatingCompany(a));
const symbols=universe.map(a=>a.symbol),snapshots={},snapshotFailures=[];
for(const batch of chunks(symbols,50)){
...
for(const failed of snapshotFailures){
  for(const batch of chunks(failed.symbols,10)){
    ...
    const q=new URLSearchParams({symbols:missing.join(','),feed:'iex'});
    try{Object.assign(snapshots,await getRetry(`https://data.alpaca.markets/v2/stocks/snapshots?${q}`,3));}catch{}
  }
}
...
const fullValidation={schemaVersion:2,generatedAt:report.generatedAt,purpose:'Every symbol that received real backtested historical validation this run (entry/stop/target geometry plus forward win-rate/sample-depth), not just the narrow qualified-optimizer winners in broad-stock-universe.json. Diagnostic/learning breadth only -- qualification for a live order still runs through the normal optimizer gates. price/ma20/ma50/ma200 added (schemaVersion 2) so scan-small-account-options.mjs can rank this full pool directly instead of only the top-30 topCandidates slice.',method:report.method,candidateCount:deep.length,candidates:deep.map(x=>({symbol:x.symbol,setupType:x.setupType,direction:x.direction,score:x.score,price:x.price,ma20:x.ma20,ma50:x.ma50,ma200:x.ma200,entry:x.entry,stop:x.stop,target1:x.target1,target2:x.target2,dollarVolume:x.dollarVolume,atrPct:x.atrPct,validation:x.validation,qualifiedForOptimizer:broadQualified.some(b=>b.symbol===x.symbol)}))};
await fs.writeFile('docs/data/full-stock-validation-pool.json',JSON.stringify(fullValidation,null,2));
