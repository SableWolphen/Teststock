...
const fullValidation={schemaVersion:1,generatedAt:report.generatedAt,purpose:'Every symbol that received real backtested historical validation this run (entry/stop/target geometry plus forward win-rate/sample-depth), not just the narrow qualified-optimizer winners in broad-stock-universe.json. Diagnostic/learning breadth only -- qualification for a live order still runs through the normal optimizer gates.',method:report.method,candidateCount:deep.length,candidates:deep.map(x=>({symbol:x.symbol,setupType:x.setupType,direction:x.direction,score:x.score,entry:x.entry,stop:x.stop,target1:x.target1,target2:x.target2,dollarVolume:x.dollarVolume,atrPct:x.atrPct,validation:x.validation,qualifiedForOptimizer:broadQualified.some(b=>b.symbol===x.symbol)}))};
