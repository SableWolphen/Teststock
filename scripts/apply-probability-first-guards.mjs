...
const policy={
  ...
  crypto:{
    ...
    dayTradeSeedLane:{enabled:true,maxOrderUsd:5,maxConcurrentPositions:1,maxHoldingHours:8,maxStopLossesPerUtcDay:2,requiredGrades:['A','A+'],requiresBrokerResidentStop:true,rule:'User-authorized crypto-only seed lane. It may bypass only the forward-shadow admission wait at a fixed $5 ceiling; every research, freshness, liquidity, spread, no-chase, Robinhood tradability, buying-power and protection gate still applies. LIVE_SUSPENDED never qualifies.'},
