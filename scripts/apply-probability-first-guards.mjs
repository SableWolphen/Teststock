...
const policy={
  ...
  objective:'Increase the probability of positive compounding while keeping stocks primary, options exceptional, crypto active around the clock, and cash acceptable. Prefer elite A stocks at normal encoded size, allow a tightly capped B best-acceptable micro-probation stock after A-tier priority whenever live capacity remains, and let crypto surface more reasonable 24/7 opportunities without removing hard risk controls. This policy cannot eliminate market loss risk.',
  stocks:{
    minGrowthQuality:85,minHistoricalSamples:12,minHistoricalWinRatePct:50,minConservativeExpectedR:0.4,minCostAdjustedConservativeExpectedR:0.35,minRewardRisk:2.0,rewardRiskQualificationTarget:'TARGET2',maxAtrPct:9,reduceSizeAboveAtrPct:5,highVolatilitySizeMultiplier:0.5,maxSinglePositionPlannedStopPct:1.5,maxCombinedNewStockPlannedStopPct:2.5,
    bestAcceptable:{enabled:true,entryTier:'B',sizeMultiplier:.25,minGrowthQuality:80,minHistoricalSamples:12,minHistoricalWinRatePct:48,minConservativeExpectedR:.3,minCostAdjustedConservativeExpectedR:.3,minRewardRisk:1.8,maxAtrPct:9,rule:'B is a micro-probation fallback capped at one-quarter of normal candidate size, not a waiver. It must still have bullish direction, positive conservative expectancy, adequate history, acceptable R:R, bounded volatility, and every downstream live account/execution/protection guard.'},
    seedLane:{enabled:true,maxOrderUsd:30,bTierMaxOrderUsd:7.5,maxConcurrentPositions:3,maxNewPositionsPerUtcDay:2,requiredEntryTier:'A',allowBTier:true,requiresPerOrderApproval:false,requiresBrokerResidentStop:false,allowsSyntheticProtection:true,existingRobinhoodCashOnly:true,agentMayInitiateDeposits:false,agentMayInitiateBankTransfers:false,marginAllowed:false,rule:'User-authorized automatic stock learning lane, expanded 2026-09-18 at user request for more trades/day and larger size. It may bypass only the forward profitability-admission wait at up to a $30 ceiling for A-tier candidates (raised from $20); a B-tier (best-acceptable) candidate is now also seed-lane eligible but stays capped at $7.50 (25% of the A-tier ceiling), matching this policy\'s existing B-tier micro-probation sizing elsewhere. Up to 3 concurrent positions and 2 new positions per UTC day (raised from 2/1). Every research, freshness, entry-zone, no-chase, decision-intelligence, buying-power, account-floor, portfolio, duplicate-order and protection gate remains mandatory. LIVE_SUSPENDED and contradictory shadow evidence never qualify. A fractional-only fill (common at this size) has no broker-resident stop available on Robinhood -- this lane uses the same agent-managed synthetic-stop protection already defined for fractional positions elsewhere (fractionalProtectionPolicy), including its tighter 0.75%-of-equity planned-stop-risk cap and mandatory active-monitoring requirement, rather than requiring an impossible broker-resident stop.'},
    dayTradeSeedLane:{enabled:true,maxOrderUsd:30,bTierMaxOrderUsd:7.5,maxConcurrentPositions:5,maxNewPositionsPerUtcDay:5,requiredEntryTier:'A',allowBTier:true,requiresPerOrderApproval:false,requiresBrokerResidentStop:false,allowsSyntheticProtection:true,existingRobinhoodCashOnly:true,agentMayInitiateDeposits:false,agentMayInitiateBankTransfers:false,marginAllowed:false,mustBeFlatBeforeMarketClose:true,entryCutoffMinutesBeforeClose:20,forcedExitStartMinutesBeforeClose:10,targetPct1:0.02,targetPct2:0.03,rule:'User-authorized same-day stock seed lane, expanded 2026-09-18 at user request for more trades/day and larger size. It reuses the swing seed candidate pool, never overlaps a swing selection, accepts entries only during the authoritative regular session with at least 20 minutes to close (matching the system-wide day-trader policy), and requires repeated forced-exit dispatches from 10 minutes before close until Robinhood confirms the position is flat. Up to a $30 ceiling for A-tier candidates (raised from $20); a B-tier (best-acceptable) candidate is now also day-trade-seed-lane eligible but stays capped at $7.50 (25% of the A-tier ceiling). Up to 5 concurrent positions and 5 new positions per UTC day (raised from 3/3). A fractional-only fill (common at this size) has no broker-resident stop available on Robinhood -- this lane uses the same agent-managed synthetic-stop protection already defined for fractional positions elsewhere (fractionalProtectionPolicy), including its tighter 0.75%-of-equity planned-stop-risk cap and mandatory active-monitoring requirement, rather than requiring an impossible broker-resident stop. Synthetic protection still requires the same <=5 minute active-monitoring cadence during the regular session as the swing lane, separate from this lane\'s own forced-exit timing. Profit target tightened 2026-09-22 (user-requested day-trade-only phase): target1/target2 for this lane use targetPct1/targetPct2 (2%/3% above entry) instead of the shared swing 1.5R/2.6R geometry, since the goal is small, frequent, same-day gains rather than a multi-day swing target -- stop/invalidation logic is unchanged.'},
    winningPositionAddOn:{enabled:true,maxOrderUsd:20,maxAddOnsPerSymbolPerUtcDay:1,requiredEntryTier:'A',minimumUnrealizedGainPct:1,requiresFreshBuyTrigger:true,requiresNoPendingEquivalentOrder:true,requiresCombinedRiskCapacity:true,requiresProtectionAtLeastAsTightAsExisting:true,averageDownAllowed:false,widenStopAllowed:false,existingRobinhoodCashOnly:true,marginAllowed:false,rule:'A fresh A-tier trigger may add at most $20 to an already-held Teststock stock only when the live position is profitable by at least 1%, no equivalent order is pending, no add-on was submitted for that symbol today, combined portfolio risk remains inside every cap, and protection is preserved or tightened. Never average down or widen the stop.'},
  ...
  crypto:{
    ...
    dayTradeSeedLane:{enabled:true,maxOrderUsd:5,maxConcurrentPositions:1,maxNewPositionsPerUtcDay:1,maxHoldingHours:8,maxStopLossesPerUtcDay:2,requiredGrades:['A+'],requiresBrokerResidentStop:true,requiresPerOrderApproval:false,executionLane:'CLAUDE_ROBINHOOD_TRADING_MCP',existingRobinhoodCashOnly:true,agentMayInitiateDeposits:false,agentMayInitiateBankTransfers:false,marginAllowed:false,rule:'User-authorized automatic crypto learning lane through Claude and the authenticated Robinhood Trading MCP. It may bypass only the forward-shadow admission wait at a fixed $5 ceiling; every research, freshness, liquidity, spread, no-chase, Robinhood tradability, buying-power, duplicate-order and protection gate still applies. LIVE_SUSPENDED never qualifies. Restricted to A+ only 2026-09-22 (user-requested: "don\'t buy crypto as a day trader unless it\'s good") -- plain A grade (generate-crypto-picks.mjs\'s grade()) requires only growthQuality>=84/score>=78/atrPct<=12 and lets thin historical evidence bypass the win-rate bar entirely (historyA: samples<6||win>=50), while A+ demands growthQuality>=92/score>=86/atrPct<=11 and real sample-backed evidence (historyAPlus: samples>=10&&win>=56) -- a materially stronger, not just technically-qualified, setup.'},
