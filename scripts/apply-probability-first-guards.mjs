...
const policy={
  ...
  stocks:{
    ...
    instructions:'Target2 is the qualification target; target1 is for partial profit/de-risking. Use cost-adjusted conservative expectancy when available so execution friction cannot be ignored.'
  },
  options:{
...
for(const budget of budgets){
  ...
  for(const row of original){
    ...
    const checks={growthQuality:Number(row.growthQuality||0)>=policy.stocks.minGrowthQuality,samples:Number(row.validation?.samples||0)>=policy.stocks.minHistoricalSamples,winRate:Number(row.validation?.winRate||0)>=policy.stocks.minHistoricalWinRatePct,expectancy:Number(row.expectancy?.conservativeExpectedR||0)>=policy.stocks.minConservativeExpectedR,costAdjustedExpectancy:costAdjusted>=policy.stocks.minCostAdjustedConservativeExpectedR,rewardRisk:Number(row.rewardRisk||0)>=policy.stocks.minRewardRisk,volatility:Number(row.atrPct??row.technicals?.atrPct??0)<=policy.stocks.maxAtrPct};
  ...
  for(const row of kept.sort((a,b)=>Number(b.portfolioOpportunityScore||0)-Number(a.portfolioOpportunityScore||0)||Number(b.growthQuality||0)-Number(a.growthQuality||0))){const risk=Number(row.estimatedLossAtStop||0);if(risk<=0||risk>remainingCombinedRisk)continue;final.push(row);remainingCombinedRisk=round(remainingCombinedRisk-risk);if(final.length>=4)break;}
  plan.allocations=final;plan.keepCashDollars=round(Math.max(0,budget-final.reduce((s,x)=>s+Number(x.allocationDollars||0),0)));plan.estimatedPortfolioStopLoss=round(final.reduce((s,x)=>s+Number(x.estimatedLossAtStop||0),0));plan.confidence=final.length?(final[0].growthQuality>=97?'A_PLUS':'A'):'CASH';plan.probabilityFirstGuard={applied:true,stockPolicy:policy.stocks,removedAllocationCount:Math.max(0,original.length-final.length)};

  const option=plan.eliteOption;
  if(option){
    const underlying=final[0]||plan.ranked?.[0]||null,costAdjusted=Number(underlying?.expectancy?.costAdjustedConservativeExpectedR??underlying?.expectancy?.conservativeExpectedR??0);
...
console.log('Applied probability-first stock/options/crypto overlay with cost-adjusted stock expectancy.');
