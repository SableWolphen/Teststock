...
test('qualified stock and crypto entries are automatic under the scheduled authorization',()=>{assert.match(executor,/Qualified stock entries/);assert.match(executor,/Crypto entries are fully automatic/);assert.match(executor,/This scheduled run is authorized to perform only the broker actions described below/);});
