...
for(const e of board.events||[]){if(e.trigger==='BUY_TRIGGER'&&e.assetClass!=='STOCK')fail.push('non-stock buy sent to Claude');if(e.trigger==='BUY_TRIGGER'&&['SHADOW_ONLY','LIVE_SUSPENDED','UNKNOWN',undefined,null].includes(e.profitabilityAdmission))fail.push(`${e.ticker}: buy lacks profitability admission`);if(e.trigger==='BUY_TRIGGER'&&e.decisionIntelligenceEligible!==true)fail.push(`${e.ticker}: buy lacks decision-intelligence pass`);}
for(const e of board.events||[]){if(e.trigger==='SEED_LANE_BUY_TRIGGER'&&(e.assetClass!=='STOCK'||e.seedLane?.eligible!==true||e.seedLane?.requiresPerOrderApproval!==false||Number(e.seedLane?.maxOrderUsd)!==20))fail.push(`${e.ticker}: invalid stock seed trigger`);if(e.trigger==='CRYPTO_SEED_LANE_BUY_TRIGGER'&&(e.assetClass!=='CRYPTO'||e.seedLane?.eligible!==true||e.seedLane?.requiresPerOrderApproval!==false||Number(e.seedLane?.maxOrderUsd)!==5))fail.push(`${e.ticker}: invalid crypto seed trigger`);}
for(const e of board.events||[]){if(e.trigger==='STOCK_DAY_TRADE_SEED_LANE_BUY_TRIGGER'&&(e.assetClass!=='STOCK'||e.dayTradeSeedLane?.eligible!==true||e.dayTradeSeedLane?.requiresPerOrderApproval!==false||Number(e.dayTradeSeedLane?.maxOrderUsd)!==20||e.dayTradeSeedLane?.mustBeFlatBeforeMarketClose!==true||Number(e.dayTradeSeedLane?.entryCutoffMinutesBeforeClose)!==30||Number(e.dayTradeSeedLane?.forcedExitStartMinutesBeforeClose)!==15||e.marketSession?.regularSession!==true||Number(e.marketSession?.minutesToClose)<30))fail.push(`${e.ticker}: invalid stock day-trade seed trigger`);if(e.trigger==='STOCK_DAY_TRADE_FORCED_EXIT'&&(e.assetClass!=='STOCK'||e.dayTradeSeedLane!==true||e.marketSession?.forcedExitDue!==true))fail.push(`${e.ticker}: invalid stock day-trade forced exit`);}
