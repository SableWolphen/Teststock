...
signal.generatorIntegrity={...(signal.generatorIntegrity||{}),traceableFeatures:{...(signal.generatorIntegrity?.traceableFeatures||{}),exitPolicyHoldoutValidation:true}};
signal.schemaVersion=Math.max(23,Number(signal.schemaVersion||0));
await fs.writeFile(signalFile,JSON.stringify(signal,null,2));await fs.writeFile('docs/data/claude-signal.json',JSON.stringify(signal,null,2));console.log(`Applied exit-policy evidence: ${policyId}, validation ${valid?'fresh':'failsafe'}.`);
