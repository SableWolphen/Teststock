...
// User-requested loosening (2026-08-23): volumeConfirm was failing on essentially every scanned
// candidate (30/30 observed live) regardless of underlying trend. Root cause -- Crypto.com's latest
// 4h candle is frequently still forming when this scan runs, so its volume is partial by
// construction and reads as "no spike" almost every time no matter what the market is doing. Fixed
// by comparing only fully-closed candles for the volume check (detected by candle-open-time age vs
// the 4h interval); price/RSI/momentum below are unaffected and still read the live last candle,
// since that best reflects the current market. The spike multiplier is also relaxed from 1.05x to
// 0.90x -- "not unusually quiet" rather than "must be actively spiking" -- since requiring an
// outright volume spike on top of every other already-aligned condition (RSI, VWAP pullback,
// momentum) was rarely all true at once even on genuinely good setups. No other grade requirement,
// liquidity floor, or risk rule changed.
//
// User-requested loosening (2026-08-23, second change, explicitly approved via AskUserQuestion --
// "Relax the 4-hour confirmation"): previously ALL FOUR of {RSI 50-70, pullback to VWAP/support,
// momentum turning up, volume confirming} had to be true simultaneously, on top of the separate
// price>=ma20*.995 trend check. Live crypto-universe.json data showed strong, liquid majors
// (BTC/USD, SOL/USD, ADA/USD, etc.) routinely had 3 of these 4 align while the market's own noise
// kept exactly one from lining up at the same instant -- so the all-or-nothing gate was rejecting
// good setups, not just weak ones. Changed to require only 3 of these 4 (any 3), while the price
// vs ma20 trend check remains a separate, still-mandatory requirement. The liquidity floor
// (dollarVolume24hReal minimums in grade()) and the crypto profitability-admission ladder
// (crypto-profitability-admission.json) are both untouched by this change -- this only affects
// which candidates can reach A/A+ setupGrade, not how much real money any admitted candidate risks.
function fourHourSetup(symbol,intra){const xs=intra[symbol]||[];if(xs.length<22)return{confirmed:false};const recent=xs.slice(-20),c=xs.map(x=>Number(x.c)),last=c.at(-1),previous=c.at(-2),ma20=sma(c,20),volume=recent.map(x=>Number(x.v||0));const FOUR_H_MS=4*3600*1000,lastOpenMs=new Date(xs.at(-1)?.t||0).getTime(),lastCandleInProgress=Number.isFinite(lastOpenMs)&&(Date.now()-lastOpenMs)<FOUR_H_MS,closedVolume=lastCandleInProgress?volume.slice(0,-1):volume,volumeAvg=avg(closedVolume.slice(0,-1)),volumeConfirm=closedVolume.length>=2&&closedVolume.at(-1)>=volumeAvg*.9;const weighted=recent.reduce((s,x)=>s+((Number(x.h)+Number(x.l)+Number(x.c))/3)*Number(x.v||0),0),volumeSum=volume.reduce((s,x)=>s+x,0),vwap=volumeSum?weighted/volumeSum:0,rsi4h=round(rsi(c),1),rsiOk=rsi4h>=50&&rsi4h<=70,pullbackToSupportOrVwap=vwap>0&&(Math.abs(last-vwap)/vwap<=.025||(Number(recent.at(-1).l)<=vwap*1.01&&last>=vwap*.995)),momentumTurnedUp=last>previous&&pct(last,c.at(-4))>0,trendOk=last>=ma20*.995,confirmationsMet=[rsiOk,pullbackToSupportOrVwap,momentumTurnedUp,volumeConfirm].filter(Boolean).length,confirmationsRequired=3,confirmed=trendOk&&confirmationsMet>=confirmationsRequired;return{confirmed,rsi4h,vwap4h:round(vwap,6),support4h:round(Math.min(ma20,vwap||ma20),6),pullbackToSupportOrVwap,momentumTurnedUp,volumeConfirm,confirmationsMet,confirmationsRequired};}
...
// Stop tightened 2026-09-21 (user-requested, following live PEPE-USD stop repair): crypto's
// volatility means a wide stop can lose real money fast, so both the ATR-based candidate and the
// max allowed per-trade risk below are pulled in tighter than before -- ATR multiplier 2.1->1.5
// (Math.max already selects the tightest of the three candidates, so this can only tighten or
// leave unchanged the resulting stop, never widen it) and the 14%->9% risk ceiling below now
// matches the ~9%-from-entry stop manually applied to the live PEPE-USD position.
function candidate(m,cal,intraday,btcTrend){const entry=m.price*1.002,intradayStop=Number(intraday.support4h||0)*.995,stop=Math.max(m.ma20,m.price-m.atr*1.5,intradayStop),risk=entry-stop;if(risk<=0)return null;const target1=entry+risk*3,target2=entry+risk*5;let q=m.score;if(intraday.confirmed)q+=5;else q-=8;if(cal.samples>=10&&cal.winRate>=56)q+=5;if(cal.samples>=8&&cal.winRate<48)q-=8;if(m.m20>30)q-=5;if(m.atrPct>12)q-=6;if(m.symbol!=='BTC/USD'&&btcTrend!==true)q-=6;q=Math.round(clamp(q,0,100));return{...m,...intraday,growthQuality:q,confirm4h:intraday.confirmed,btcTrendSupport:m.symbol==='BTC/USD'?true:btcTrend,validation:cal,entry:round(entry,6),stop:round(stop,6),target1:round(target1,6),target2:round(target2,6),rewardRisk1:3,rewardRisk2:5};}
// intradayOk mirrors fourHourSetup()'s own `confirmed` field (x.confirm4h) rather than
// re-deriving a stricter all-4 AND here -- previously this duplicate check silently required all
...
// Liquidity floor lowered 2026-09-12 (user-requested): $250K/$2M -> $80K for both tiers. Position
// sizes here are tiny ($5-20), so the concern is reliable pricing/exit near the stop on a thin
// coin, not this account's own market impact. No historical dollar-volume-per-trade data exists
// to back-test this the way the BTC-trend requirement was validated -- treat as a real tradeoff,
// not a proven-safe change.
function grade(x){const samples=Number(x.validation?.samples||0),win=Number(x.validation?.winRate||0),historyAPlus=samples>=10&&win>=56,historyA=samples<6||win>=50,btcOk=x.symbol==='BTC/USD'||x.btcTrendSupport===true,intradayOk=x.confirm4h===true;if(x.growthQuality>=92&&x.score>=86&&intradayOk&&historyAPlus&&x.atrPct<=11&&btcOk&&x.dollarVolume24hReal>=80_000)return'A+';if(x.growthQuality>=84&&x.score>=78&&intradayOk&&historyA&&x.atrPct<=12&&btcOk&&x.dollarVolume24hReal>=80_000)return'A';return'NO_TRADE';}
