...
const tierLabel=ticker=>planByTicker.get(ticker)?.entryTierLabel||(tierFor(ticker)==='B'?'BEST_ACCEPTABLE_MICRO':'ELITE');
const tierSize=ticker=>Number(planByTicker.get(ticker)?.entryTierSizeMultiplier??(tierFor(ticker)==='B'?.25:1));
...
const rounds=[
  ...
  {round:'TOP_100',input:broad.qualifiedForOptimizer||100,output:liveQueue.length,rule:'Apply A-tier elite gates first, then a 25% B best-acceptable micro tier with positive expectancy/history/R:R floors. Hard account, price, spread, gap, correlation and protection gates never relax.'},
  {round:'LIVE_FINAL',input:liveQueue.length,output:champion?1:0,rule:'Choose live A candidates first at up to normal encoded size. If no A survives, choose the strongest live B candidate at micro-probation size capped at 25%. If it fails before submission, try the next eligible fallback. Hold cash only when no A or B survives every hard guard.'}
];
const tournament={
  schemaVersion:5,source:'TESTSTOCK_UP_TO_20000_STOCK_KNOCKOUT',generatedAt:new Date().toISOString(),
  ...
  policy:{
    ...
    tierPriority:['A','B'],
    aTier:'ELITE_NORMAL_ENCODED_SIZE',
    bTier:'BEST_ACCEPTABLE_MICRO_25_PERCENT_SIZE',
    ...
    safetyNote:'The B tier increases opportunity frequency using a tightly capped micro position without turning failed hard guards into passes. No trade is forced when both tiers fail.'
  },
...
signal.stockTournament={
  ...
  instructions:'Use every legitimate active/tradable operating-company stock available from the connected U.S. equity universe, up to 20,000 symbols. Prefer A-tier elite candidates at up to their normal encoded size. If no A candidate survives every live guard, buy the strongest B-tier best-acceptable candidate at no more than 25% micro-probation size. If it fails before submission, try the next eligible fallback in the same run. Never relax hard account, freshness, pricing, spread, gap, correlation or protection rules just to ensure a trade.'
};
signal.generatorIntegrity={...(signal.generatorIntegrity||{}),traceableFeatures:{...(signal.generatorIntegrity?.traceableFeatures||{}),stockKnockoutTournament:true,bestAcceptableStockTier:true,bTierMicroProbation:true}};
