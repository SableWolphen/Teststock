...
const stockSeed=s.probabilityFirstPolicy?.stocks?.seedLane;if(stockSeed?.enabled!==true||Number(stockSeed?.maxOrderUsd)!==20||Number(stockSeed?.maxConcurrentPositions)!==2||Number(stockSeed?.maxNewPositionsPerUtcDay)!==1||stockSeed?.requiresPerOrderApproval!==false||stockSeed?.requiresBrokerResidentStop!==false||stockSeed?.allowsSyntheticProtection!==true||stockSeed?.existingRobinhoodCashOnly!==true||stockSeed?.agentMayInitiateDeposits!==false||stockSeed?.agentMayInitiateBankTransfers!==false||stockSeed?.marginAllowed!==false)fail.push('stock seed lane bounds');
const stockDaySeed=s.probabilityFirstPolicy?.stocks?.dayTradeSeedLane;if(stockDaySeed?.enabled!==true||Number(stockDaySeed?.maxOrderUsd)!==20||Number(stockDaySeed?.maxConcurrentPositions)!==3||Number(stockDaySeed?.maxNewPositionsPerUtcDay)!==3||stockDaySeed?.requiredEntryTier!=='A'||stockDaySeed?.requiresPerOrderApproval!==false||stockDaySeed?.requiresBrokerResidentStop!==false||stockDaySeed?.allowsSyntheticProtection!==true||stockDaySeed?.existingRobinhoodCashOnly!==true||stockDaySeed?.agentMayInitiateDeposits!==false||stockDaySeed?.agentMayInitiateBankTransfers!==false||stockDaySeed?.marginAllowed!==false||stockDaySeed?.mustBeFlatBeforeMarketClose!==true||Number(stockDaySeed?.entryCutoffMinutesBeforeClose)!==20||Number(stockDaySeed?.forcedExitStartMinutesBeforeClose)!==10)fail.push('stock day-trade seed lane bounds');
