...
function buildStock(plan){
  if(plan?.error)return{overallAction:'DO_NOT_TRADE',reason:plan.error};
  const f=freshness(plan);
  const stocks=(plan.allocations||[]).map((r,i)=>({
    rank:i+1,ticker:r.symbol,sector:r.sector||null,action:stockState(r,f),
  ...
  return {
    overallAction,reason,confidence:plan.confidence,signalAsOf:plan.asOf,
...
function buildCrypto(plan){
  ...
  const orders=(plan.allocations||[]).map((r,i)=>{
    ...
    return{rank:i+1,ticker:r.symbol,action,allocationPctOfRobinhoodBuyingPower:pctOfReference(r.allocationDollars),scanPrice:round(r.price,6),minimumEntry:round(r.entry,6),maximumEntry:round(Number(r.entry||0)*1.02,6),stop:round(r.stop,6),target1:round(r.target1,6),target2:round(r.target2,6),plannedStopRiskPctOfRobinhoodBuyingPower:pctOfReference(r.estimatedLossAtStop),rewardRisk:r.rewardRisk1,growthQuality:r.growthQuality,confirm4h:r.confirm4h,historicalWinRate:r.validation?.winRate??null,historicalSamples:r.validation?.samples??null};
  });
...
try{stockPlan=buildStock(await read(path.join(dataDir,`growth-plan-${REFERENCE_BUDGET}.json`)));}catch(error){stockPlan={overallAction:'DO_NOT_TRADE',reason:`Growth plan unavailable: ${error.message}`};}
try{cryptoPlan=buildCrypto(await read(path.join(dataDir,`crypto-plan-${REFERENCE_BUDGET}.json`)));}catch(error){cryptoPlan={overallAction:'DO_NOT_TRADE',reason:`Crypto plan unavailable: ${error.message}`};}

const signal={
  schemaVersion:6,source:'Teststock',purpose:'Account-aware growth autopilot. Teststock supplies percentages and trade gates; the trading agent sizes from the live Robinhood Agentic account.',generatedAt:new Date().toISOString(),
  ...
  hardRules:[
    ...
    'For stocks, allocationPctOfRobinhoodBuyingPower is the maximum allocation. For options, maxRiskPctOfRobinhoodBuyingPower is the maximum premium/risk allocation.',
...
await fs.writeFile(path.join(dataDir,'crypto-signal.json'),JSON.stringify({schemaVersion:2,generatedAt:signal.generatedAt,sizing:signal.sizing,cryptoPlan},null,2));
console.log('Generated Teststock agent signal schema v6 with live Robinhood sizing');
