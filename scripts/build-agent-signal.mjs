...
function buildStock(plan){
  ...
  const option=elite.passed?{
    ...
    maxRiskPctOfRobinhoodBuyingPower:round(Math.min(Number(plan.policy?.maxOptionPremiumPct||8),pctOfReference(elite.contract?.maxRisk)),2),
...
function buildCrypto(plan){
  ...
  const orders=(plan.allocations||[]).map((r,i)=>{
    ...
    return{assetClass:'CRYPTO',rank:i+1,ticker:r.symbol,setupGrade:r.setupGrade||null,action,allocationPctOfRobinhoodBuyingPower:pctOfReference(r.allocationDollars),scanPrice:round(r.price,6),minimumEntry:round(r.entry,6),maximumEntry:round(Number(r.entry||0)*1.02,6),stop:round(r.stop,6),target1:round(r.target1,6),target2:round(r.target2,6),plannedStopRiskPctOfRobinhoodBuyingPower:pctOfReference(r.estimatedLossAtStop),rewardRisk:r.rewardRisk1,growthQuality:r.growthQuality,confirm4h:r.confirm4h,historicalWinRate:r.validation?.winRate??null,historicalSamples:r.validation?.samples??null};
  });
  const buyZone=orders.filter(x=>x.action==='SIGNAL_ONLY_BUY_ZONE');
  return{assetClass:'CRYPTO',overallAction:f.stale?'DO_NOT_TRADE':buyZone.length?'CRYPTO_BUY_ZONE':'WAIT_FOR_CRYPTO_TRIGGER',reason:f.stale?'Crypto signal is stale.':buyZone.length?'An A/A+ crypto setup is in its buy zone.':'No A/A+ crypto setup is currently in its allowed buy zone.',confidence:plan.confidence,signalAsOf:plan.asOf,expiresAt:plan.asOf?new Date(new Date(plan.asOf).getTime()+30*60000).toISOString():null,policy:{...plan.policy,sizingMode:'ROBINHOOD_EXISTING_CASH_ONLY_WITH_CAPITAL_LADDER'},execution:{mode:'USE_ONLY_IF_CONNECTED_ROBINHOOD_TOOLS_SUPPORT_CRYPTO',note:'At runtime, use crypto order tools only if the connected Robinhood MCP actually exposes them.'},cryptoOrders:orders};
}
...
const signal={
  ...
  profitLock:{
    ...
    floors:[
      {minEquity:0,maxEquity:124.99,protectedEquityFloor:0},
      {minEquity:125,maxEquity:149.99,protectedEquityFloor:100},
      {minEquity:150,maxEquity:199.99,protectedEquityFloor:125},
      {minEquity:200,maxEquity:299.99,protectedEquityFloor:160},
      {minEquity:300,maxEquity:499.99,protectedEquityFloor:225},
      {minEquity:500,maxEquity:null,protectedEquityFloorPctOfCurrentEquity:75}
    ],
