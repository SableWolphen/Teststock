...
if(seeds.some(x=>x.trigger==='SEED_LANE_BUY_TRIGGER'&&x.requiresBrokerResidentStop!==false)) fail('stock swing seed-lane must use synthetic protection, not an impossible broker-resident stop on a fractional order');
if(seeds.some(x=>x.trigger!=='SEED_LANE_BUY_TRIGGER'&&x.requiresBrokerResidentStop!==true)) fail('day-trade/crypto seed-lane protection bounds');
