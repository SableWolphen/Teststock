...
if(!secUA)throw new Error('Missing SEC_USER_AGENT');
...
const tickerMap=await getJson('https://www.sec.gov/files/company_tickers.json',{'User-Agent':secUA,'Accept-Encoding':'gzip, deflate','Host':'www.sec.gov'});
...
let corporate={};
try{const q=new URLSearchParams({symbols:symbols.join(','),start,end,limit:'1000',data_quality:'all'});corporate=await getJson(`https://data.alpaca.markets/v1/corporate-actions?${q}`,alpacaHeaders);}catch(e){console.warn('Corporate-action enrichment unavailable:',e.message);}
...
for(const budget of [50,100,200,500]){const file=`docs/data/latest-${budget}.json`,data=await read(file);if(!data)continue;data.recommendations=(data.recommendations||[]).map(r=>enriched[r.symbol]?{...r,...enriched[r.symbol]}:r);data.finalistEnrichment={generatedAt:new Date().toISOString(),symbolsEnriched:Object.keys(enriched).length,sources:['SEC company facts/submissions','Alpaca corporate actions'],earningsCalendar:'UNKNOWN_FAIL_CLOSED'};await fs.writeFile(file,JSON.stringify(data,null,2));}
...
console.log(`Enriched ${Object.keys(enriched).length} stock finalists with SEC fundamentals and Alpaca corporate actions.`);
