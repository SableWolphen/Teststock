...
// Stock seed lane (2026-08-26, expanded 2026-08-26 to support up to N concurrent distinct
// candidates instead of one): computed entirely separately from the BUY_TRIGGER approval-batch
// pipeline above so the seed cap can never be blended with normal multi-stock batch sizing or
// counted against MAX_NEW_BUYS_PER_DISPATCH. Presented only when no exit event exists (exits
// always win, same stopPriorityRule as everything else) and the board is healthy/fresh. Every
// entry still requires the user's own per-order approval before any order, exactly like every
// other stock buy -- this dispatch packet never auto-submits, unlike crypto's seed lane. This is
// a pre-filter only; the execution-check must still re-verify live concurrency (against
// real-trade-journal.json OPEN seedLane===true entries) and same-day-stop exclusion before
// presenting or submitting any of these, since apply-profitability-admission.mjs's own filter can
// be several scans old by the time this dispatch is consumed.
...
const compactSeed=e=>({
  ...
  reason:e.reason,
  packet:`${e.ticker} | SEED_LANE_BUY_TRIGGER | observed ${e.observedPrice ?? 'UNKNOWN'} | capped at $${Number(e.seedLane?.maxOrderUsd||5)} | requires per-order approval`
});
...
const out={
  ...
  pendingAction,approvalCandidates,approvalBatchId,fallbackActions,seedLaneCandidates,
  ...
  actionInstruction:'Atomically claim each candidate fingerprint in private execution state before any broker action. Exit events always take priority and exclude new buys AND every seed-lane candidate. For BUY_TRIGGER dispatches, present approvalCandidates together as one exact approval batch when practical. The user may approve the entire current batch in one explicit response, but that approval must be bound to approvalBatchId and may never authorize future or newly-added candidates. After approval, recheck each candidate independently immediately before submission. After each approved fill, recompute remaining cash, account floor, total deployed capital, planned stop risk, correlation, trade-frequency limits, protection capability and concurrent-position capacity before considering the next candidate. If a candidate fails before submission, skip it and continue to the next approved candidate in the same batch. Never force a trade or fill every available slot. Reconcile ambiguous or partial results by client order ID; never retry an uncertain submission as a new order. If seedLaneCandidates is non-empty (and no exit event exists), each entry is entirely separate from approvalCandidates/approvalBatchId -- capped at its own maxOrderUsd, up to maxConcurrentPositions total concurrent seed positions. Before presenting or submitting any of them, read real-trade-journal.json live and recompute: which tickers currently have an OPEN seedLane===true entry (occupied slots), and which tickers closed a seedLane===true position on a STOP earlier this UTC day (excluded for the rest of today, not paused -- move to the next distinct candidate instead). Only present seed candidates for open slots on tickers not already excluded, each requiring its own explicit per-order user approval before any order; never auto-submitted, never combined with a normal BUY_TRIGGER batch, never exceed maxConcurrentPositions concurrently open seed positions.',
...
console.log(pendingAction?`New actionable dispatch: ${pendingAction.packet}; ${approvalCandidates.length} approval candidate(s), batch ${approvalBatchId||'NONE'}.`:seedLaneCandidates.length?`Seed-lane dispatch: ${seedLaneCandidates.map(x=>x.packet).join(' || ')}.`:'Dispatch idle or suppressed: no new safe brokerage action.');
