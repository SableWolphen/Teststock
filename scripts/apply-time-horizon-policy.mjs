...
const commonRotation={
  ...
  sameSymbolReentryCooldownMinutes:10,
...
function stockPolicy(x){return {
  ...
  entryEvidence:['authoritative regular NYSE session is open','at least 20 minutes remain before the actual New York close','fresh live liquidity and spread pass','current price is inside the encoded entry zone and does not chase','intraday VWAP/support/resistance and momentum context remain valid'],
...
console.log(`Active day-trader execution friction reduced: stocks=${stocks.liveQueue.length+stocks.researchFinalists.length}; crypto=${crypto.ranked.length}; fast entry/exit policy active`);