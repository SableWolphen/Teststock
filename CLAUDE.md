...
- Robinhood Crypto API: authoritative crypto account, order, fill, and protection lane. The crypto autopilot also publishes a read-only Robinhood pair-tradability and live best-bid/ask cross-check (`docs/data/crypto-robinhood-cross-check.json`) for the current research/qualified champion and fallbacks on every run, surfaced on the dashboard next to the Crypto.com-sourced crypto card. This is diagnostic only, contributes zero live ranking weight, and never places, cancels, or modifies an order; Crypto.com remains the primary crypto research/bars/liquidity source. Stocks are unaffected -- Robinhood stock verification already happens at approval/submission time through the existing per-order Claude-mediated path.
